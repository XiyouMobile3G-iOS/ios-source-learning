...
> 同目录的 `AGENTS.md` 只是指向本文件的指针，内容等价，**不要重复读取**。各模块目录下同样成对存在 `CLAUDE.md`（正文）/ `AGENTS.md`（指针）。

## 模块记忆索引（渐进式披露）

本文件只给全局背景与构建/测试命令。**动手改某个模块前，先读该模块的 `CLAUDE.md`**——里面是文件级目录表和关键符号，用于快速定位源码，避免全仓库扫描。

| 模块 | 记忆文件 | 覆盖内容 |
|---|---|---|
| runtime 主体 | [`runtime/CLAUDE.md`](./runtime/CLAUDE.md) | 「按任务定位」总表、三层头文件划分、`objc-runtime-new.h` 数据结构地图、类实现化/category/ivar 的主流程、NSObject.mm 分区 |
| 消息发送汇编 | [`runtime/Messengers.subproj/CLAUDE.md`](./runtime/Messengers.subproj/CLAUDE.md) | 各架构 `.s` 文件、`objc_msgSend` 系列入口点行号、与 C 侧结构布局的耦合点 |
| 线程与锁 | [`runtime/Threading/CLAUDE.md`](./runtime/Threading/CLAUDE.md) | 后端选择与 mixin 组装、`tls_key` 分配表、全局锁声明位置与加锁顺序、fork 安全 |
| C 侧测试套件 | [`test/CLAUDE.md`](./test/CLAUDE.md) | `test.pl` 用法、`TEST_*` 指令全表、`test.h`/`testroot.i` 宏、测试命名规律 |
| Swift overlay | [`ObjectiveC/CLAUDE.md`](./ObjectiveC/CLAUDE.md) | `ObjectiveC.swift` 内容地图、xcconfig 里绕 bug 的非显然设置、XCTest 跑法 |
| 调试工具 | [`objcdt/CLAUDE.md`](./objcdt/CLAUDE.md) | **本 drop 中是空壳**，勿浪费时间；替代调试手段 |
...
写测试的完整规则（`TEST_*` 指令表、`test.h`/`testroot.i` 宏、必须打印 `OK: <name>`）见 [`test/CLAUDE.md`](./test/CLAUDE.md)。

## 全局地图

只记"哪一层做什么"，具体文件与符号见对应模块记忆：

- `runtime/` —— libobjc 全部实现。核心是 `objc-runtime-new.mm/.h`（类实现化、category、非脆弱 ivar、全部数据结构）；引用计数快路径在 `objc-object.h` + `isa.h`，**不在** `NSObject.mm`。→ [`runtime/CLAUDE.md`](./runtime/CLAUDE.md)
- `runtime/Messengers.subproj/` —— `objc_msgSend` 各架构汇编，与 `cache_t`/`objc_class` 的内存布局强耦合。→ [模块记忆](./runtime/Messengers.subproj/CLAUDE.md)
- `runtime/Threading/` —— 锁/TLS 抽象层，后端由 `objc-config.h` 的 `OBJC_THREADING_PACKAGE` 选择。→ [模块记忆](./runtime/Threading/CLAUDE.md)
- `test/` —— C 侧主测试套件（Perl 驱动）。→ [模块记忆](./test/CLAUDE.md)
- `ObjectiveC/` + `ObjectiveCTests/` —— Swift overlay 与其 XCTest。→ [模块记忆](./ObjectiveC/CLAUDE.md)
- `objcdt/` —— 调试工具，本 drop 中是空壳。→ [模块记忆](./objcdt/CLAUDE.md)
...
头文件的三层可见性（公开 / SPI / 内部）以及 `objc.xcconfig`、`runtime/Module*/` modulemap+apinotes 的配合规则，见 [`runtime/CLAUDE.md`](./runtime/CLAUDE.md) 的「分层」一节——新增 API 时这几处要一起改。
